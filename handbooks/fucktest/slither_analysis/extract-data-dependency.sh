...
echo -e "${BLUE}=== BakerFi 数据依赖图提取工具 ===${NC}\n"

# 切换到项目根目录
cd "$(dirname "$0")/../../.."
PROJECT_ROOT=$(pwd)
OUTPUT_DIR="handbooks/fucktest/slither_analysis/data-dependency"
...
OUTPUT_FILE="${OUTPUT_DIR}/data-dependency-${TIMESTAMP}.txt"

echo -e "${YELLOW}📊 生成数据依赖报告...${NC}"
echo "（这可能需要几分钟时间...）"
echo ""

# 运行 data-dependency printer
slither . \
